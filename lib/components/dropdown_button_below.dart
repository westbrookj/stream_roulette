...
class _DropdownMenuItemButtonState<T> extends State<_DropdownMenuItemButton<T>> {
  ...
  void _handleOnTap() {
    final DropdownBelowMenuItem<T> dropdownMenuItem = widget.route.items[widget.itemIndex].item;
...
class _DropdownRoute<T> extends PopupRoute<_DropdownRouteResult<T>> {
  ...
  _MenuLimits getMenuLimits(Rect buttonRect, double availableHeight, int index) {
    ...
    final double selectedItemOffset = -50.0;
...
class _DropdownRoutePage<T> extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // DropdownButtonBelow.itemHeight is specified or DropdownButtonBelow.itemHeight is null
...
class _MenuItem<T> extends SingleChildRenderObjectWidget {
  ...
  final DropdownBelowMenuItem<T> item;
...
// The container widget for a menu item created by a [DropdownButtonBelow]. It
// provides the default configuration for [DropdownBelowMenuItem]s, as well as a
// [DropdownButtonBelow]'s hint and disabledHint widgets.
...
/// An item in a menu created by a [DropdownButtonBelow].
///
/// The type `T` is the type of the value the entry represents. All the entries
/// in a given menu must represent values with consistent types.
class DropdownBelowMenuItem<T> extends _DropdownMenuItemContainer {
  /// Creates an item for a dropdown menu.
  ///
  /// The [child] argument is required.
  const DropdownBelowMenuItem({
    Key key,
    this.onTap,
    this.value,
  ...
  ///
  /// Eventually returned in a call to [DropdownButtonBelow.onChanged].
  final T value;
}

/// An inherited widget that causes any descendant [ DropdownButtonBelow]
/// widgets to not include their regular underline.
///
/// This is used by [DataTable] to remove the underline from any
/// [DropdownButtonBelow] widgets placed within material data tables, as
/// required by the material design specification.
class DropdownButtonBelowHideUnderline extends InheritedWidget {
  /// Creates a [DropdownButtonBelowHideUnderline]. A non-null [child] must
  /// be given.
  const DropdownButtonBelowHideUnderline({
    Key key,
    @required Widget child,
  }) : assert(child != null),
        super(key: key, child: child);

  /// Returns whether the underline of [DropdownButtonBelow] widgets should
  /// be hidden.
  static bool at(BuildContext context) {
    return context.dependOnInheritedWidgetOfExactType<DropdownButtonBelowHideUnderline>() != null;
  }

  @override
  bool updateShouldNotify(DropdownButtonBelowHideUnderline oldWidget) => false;
}
...
/// Typically, an enum is used. Each [DropdownBelowMenuItem] in [items] must be
...
///
/// This sample shows a `DropdownButtonBelow` with a large arrow icon,
...
/// @override
/// Widget build(BuildContext context) {
///   return DropdownButtonBelow<String>(
///     value: dropdownValue,
...
///       .map<DropdownBelowMenuItem<String>>((String value) {
///         return DropdownBelowMenuItem<String>(
///           value: value,
///           child: Text(value),
///         );
///       })
...
/// See also:
///
///  * [DropdownBelowMenuItem], the class used to represent the [items].
///  * [DropdownButtonBelowHideUnderline], which prevents its descendant dropdown buttons
///    from displaying their underlines.
///  * [RaisedButton], [FlatButton], ordinary buttons that trigger a single action.
///  * <https://material.io/design/components/menus.html#dropdown-menu>
class DropdownButtonBelow<T> extends StatefulWidget {
  ...
  DropdownButtonBelow({
    ...
    // DropdownButtonFormField.
      ...
      items.where((DropdownBelowMenuItem<T> item) {
        return item.value == value;
      }).length == 1,
  "There should be exactly one item with [DropdownButtonBelow]'s value: "
      '$value. \n'
      'Either zero or 2 or more [DropdownBelowMenuItem]s were detected '
      'with the same value',
  ),
  ...
  /// be displayed.
  final List<DropdownBelowMenuItem<T>> items;

  /// The value of the currently selected [DropdownBelowMenuItem].
  ///
  ...
  /// [DropdownBelowMenuItem]s in [items].
  ///
  /// When a [DropdownBelowMenuItem] is selected, the widget that will be displayed
  /// from the list corresponds to the [DropdownBelowMenuItem] of the same index
  /// in [items].
  ///
  /// {@tool dartpad --template=stateful_widget_scaffold}
  ///
  /// This sample shows a `DropdownButtonBelow` with a button with [Text] that
  /// corresponds to but is unique from [DropdownBelowMenuItem].
  ///
  /// ```dart
  ...
  ///     child: DropdownButtonBelow<String>(
  ///       value: selectedItem,
  ...
  /// {@end-tool}
  ///
  /// If this callback is null, the [DropdownBelowMenuItem] from [items]
  ...
  /// {@tool dartpad --template=stateful_widget_scaffold}
  ///
  /// This sample shows a `DropdownButtonBelow` with a dropdown button text style
  ...
  ///         }).toList();
  ///       },
  ///       items: options.map<DropdownBelowMenuItem<String>>((String value) {
  ///         return DropdownBelowMenuItem<String>(
  ///           value: value,
  ///           child: Text(value),
  ///         );
...
class _DropdownButtonState<T> extends State<DropdownButtonBelow<T>> with WidgetsBindingObserver {
  int _selectedIndex;
  ...
  @override
  void didUpdateWidget(DropdownButtonBelow<T> oldWidget) {
  ...
  void _updateSelectedIndex() {
    ...
    assert(widget.value == null ||
        widget.items.where((DropdownBelowMenuItem<T> item) => item.value == widget.value).length == 1);
    _selectedIndex = null;
  ...
  Widget build(BuildContext context) {
    ...
    if (!DropdownButtonBelowHideUnderline.at(context)) {
...
/// A convenience widget that makes a [DropdownButtonBelow] into a [FormField].
class DropdownButtonBelowFormField<T> extends FormField<T> {
  /// Creates a [DropdownButtonBelow] widget that is a [FormField], wrapped in an
  /// [InputDecorator].
  ///
  ...
  DropdownButtonBelowFormField({
    Key key,
    @required List<DropdownBelowMenuItem<T>> items,
          ...
          return Focus(
            ...
            child: Builder(builder: (BuildContext context) {
              return InputDecorator(
                ...
                child: DropdownButtonBelowHideUnderline(
                  child: DropdownButtonBelow<T>(
                    items: items,
...
class _DropdownButtonFormFieldState<T> extends FormFieldState<T> {
  @override
  DropdownButtonBelowFormField<T> get widget => super.widget as DropdownButtonBelowFormField<T>;
  ...
  @override
  void didUpdateWidget(DropdownButtonBelowFormField<T> oldWidget) {
    super.didUpdateWidget(oldWidget);
